...
include "../../includes/apoteker/header.php";
...
// Query untuk Daftar Transaksi Obat Bulan Ini
$bulan_ini = date('Y-m');
$query_transaksi_obat = mysqli_query($conn, "
    SELECT transaksi_obat.*, obat.nama_obat 
    FROM transaksi_obat 
    JOIN obat ON transaksi_obat.id_obat = obat.id_obat 
    WHERE DATE_FORMAT(tanggal, '%Y-%m') = '$bulan_ini' 
    ORDER BY tanggal DESC 
    LIMIT 5
");

// Query untuk Laporan Transaksi Obat Hari Ini
$query_transaksi_obat_hari_ini = mysqli_query($conn, "
    SELECT transaksi_obat.*, obat.nama_obat 
    FROM transaksi_obat 
    JOIN obat ON transaksi_obat.id_obat = obat.id_obat 
