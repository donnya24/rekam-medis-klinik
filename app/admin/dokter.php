...
// Fetch doctor data with user account
$sql = "SELECT d.*, u.username 
        FROM dokter d 
        LEFT JOIN user u ON d.akun_dokter = u.username";
$result = $conn->query($sql);
if (!$result) {
    die("Query failed: " . mysqli_error($conn));
}

// Proses penghapusan data dokter yang dipilih
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'deleteSingle' && isset($_POST['id'])) {
        $id = $_POST['id']; // Ambil ID dokter yang akan dihapus

        // Mulai transaksi
        $conn->begin_transaction();
        try {
            // Query DELETE untuk menghapus satu data dokter
...
<div id="wrapper">
    <div id="page-wrapper">
        <div id="page-inner">
            <h1 class="page-header text-center">Data Dokter</h1>
            <div class="button-container">
                <a href="edit_dokter.php?action=tambah" class="btn btn-primary mb-3">Tambah Dokter</a>
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr>";
                            echo "<td class='checkboxColumn' style='display: none;'><input type='checkbox' class='selectItem' value='" . htmlspecialchars($row['id_dokter']) . "'></td>";
                            echo "<td>" . $no . "</td>";
                            echo "<td>" . htmlspecialchars($row['nama_dokter']) . "</td>";
                            echo "<td>" . htmlspecialchars($row['akun_dokter']) . "</td>";
                            ...
                            echo "<td class='aksi-cell'> <!-- Tambahkan class aksi-cell -->
                                    <a href='edit_dokter.php?action=edit&id=" . htmlspecialchars($row['id_dokter']) . "' class='btn btn-warning btn-sm'>Edit</a>
                                    <a href='dokter.php' class='btn btn-danger btn-sm' onclick='deleteSingle(" . htmlspecialchars($row['id_dokter']) . ")'>Hapus</a>
                                  </td>";
                            echo "</tr>";
                            $no++;
                        }
                    } else {
                        echo "<tr><td colspan='11' class='text-center'>Tidak ada data dokter.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
