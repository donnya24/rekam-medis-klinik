...
// Fetch admin data with user account
$sql = "SELECT a.*, u.username 
        FROM administrasi a 
        LEFT JOIN user u ON a.akun_administrasi = u.username";
$result = $conn->query($sql);
if (!$result) {
    die("Query failed: " . mysqli_error($conn));
}

// Proses penghapusan data administrasi yang dipilih
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'deleteSingle' && isset($_POST['id'])) {
        $id = $_POST['id']; // Ambil ID administrasi yang akan dihapus

        // Mulai transaksi
        $conn->begin_transaction();
        try {
            // Query DELETE untuk menghapus satu data administrasi
            $sql = "DELETE FROM administrasi WHERE id_administrasi = ?";
...
<head>
    ...
    <title>Data Petugas Administrasi</title>
...
<div id="wrapper">
    <div id="page-wrapper">
        <div id="page-inner">
            <h1 class="page-header text-center">Data Petugas Administrasi</h1>
            <div class="button-container">
                <a href="edit_administrasi.php?action=tambah" class="btn btn-primary mb-3">Tambah Petugas Administrasi</a>
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr>";
                            echo "<td class='checkboxColumn' style='display: none;'><input type='checkbox' class='selectItem' value='" . htmlspecialchars($row['id_administrasi']) . "'></td>";
                            echo "<td>" . $no . "</td>";
                            echo "<td>" . htmlspecialchars($row['nama_administrasi']) . "</td>";
                            echo "<td>" . htmlspecialchars($row['akun_administrasi']) . "</td>";
                            ...
                            echo "<td class='aksi-cell'> <!-- Tambahkan class aksi-cell -->
                                    <a href='edit_administrasi.php?action=edit&id=" . htmlspecialchars($row['id_administrasi']) . "' class='btn btn-warning btn-sm'>Edit</a>
                                    <a href='administrasi.php' class='btn btn-danger btn-sm' onclick='deleteSingle(" . htmlspecialchars($row['id_administrasi']) . ")'>Hapus</a>
                                  </td>";
                            echo "</tr>";
                            $no++;
                        }
                    } else {
                        echo "<tr><td colspan='10' class='text-center'>Tidak ada data petugas administrasi.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
...
<script>
    ...
    document.getElementById('deleteSelected').addEventListener('click', function() {
        ...
        if (selectedIds.length > 0 && confirm("Yakin ingin menghapus data yang dipilih?")) {
            fetch('hapus_semua_administrasi.php', {
                method: 'POST',
