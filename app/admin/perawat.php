...
// Fetch nurse data with user account
$sql = "SELECT p.*, u.username 
        FROM perawat p 
        LEFT JOIN user u ON p.akun_perawat = u.username";
$result = $conn->query($sql);
if (!$result) {
    die("Query failed: " . mysqli_error($conn));
}

// Proses penghapusan data perawat yang dipilih
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'deleteSingle' && isset($_POST['id'])) {
        $id = $_POST['id']; // Ambil ID perawat yang akan dihapus

        // Mulai transaksi
        $conn->begin_transaction();
        try {
            // Query DELETE untuk menghapus satu data perawat
            $sql = "DELETE FROM perawat WHERE id_perawat = ?";
...
<div id="wrapper">
    <div id="page-wrapper">
        <div id="page-inner">
            <h1 class="page-header text-center">Data Perawat</h1>
            <div class="button-container">
                <a href="edit_perawat.php?action=tambah" class="btn btn-primary mb-3">Tambah Perawat</a>
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr>";
                            echo "<td class='checkboxColumn' style='display: none;'><input type='checkbox' class='selectItem' value='" . htmlspecialchars($row['id_perawat']) . "'></td>";
                            echo "<td>" . $no . "</td>";
                            echo "<td>" . htmlspecialchars($row['nama_perawat']) . "</td>";
                            echo "<td>" . htmlspecialchars($row['akun_perawat']) . "</td>";
                            ...
                            echo "<td class='aksi-cell'> <!-- Tambahkan class aksi-cell -->
                                    <a href='edit_perawat.php?action=edit&id=" . htmlspecialchars($row['id_perawat']) . "' class='btn btn-warning btn-sm'>Edit</a>
                                    <a href='perawat.php' class='btn btn-danger btn-sm' onclick='deleteSingle(" . htmlspecialchars($row['id_perawat']) . ")'>Hapus</a>
                                  </td>";
                            echo "</tr>";
                            $no++;
                        }
                    } else {
                        echo "<tr><td colspan='10' class='text-center'>Tidak ada data perawat.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
